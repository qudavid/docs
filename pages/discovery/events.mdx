...
- **Venue selectors:** `event_uid[]`, `polymarket_slug[]`, and `kalshi_event_ticker[]` are combined with OR logic across parameter types. Example: `polymarket_slug=slug1&kalshi_event_ticker=ticker1` returns events matching either selector. Results are deduplicated by `event_uid` (if multiple selectors resolve to the same event, it appears once).
...
# 3b) Mix venue selectors (Polymarket slug + Kalshi ticker)
# Returns events matching either selector, deduplicated by event_uid
curl -s -H "Authorization: Bearer $PREDX_TOKEN" \
  "https://api.predexon.com/v1/events?polymarket_slug=mlb-nyy-bos-2025-07-12&kalshi_event_ticker=KXNFLGAME-25JUL13LADSFG"
