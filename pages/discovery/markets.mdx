...
> **GET `/v1/markets`** — Discover normalized markets across venues. Each market belongs to an event and includes venue-native identifiers for accessing venue-specific APIs.
...
| `polymarket_slug[]` | string[] | Polymarket event slug (from events' `venue_ids.polymarket[]`). Format: lowercase-hyphenated. Repeatable. |
| `polymarket_market_id[]` | string[] | Polymarket market ID. Format: numeric string (e.g., `123456789`). Repeatable. |
| `kalshi_event_ticker[]` | string[] | Kalshi event ticker (from events' `venue_ids.kalshi[]`). Format: uppercase alphanumeric. Repeatable. |
...
- **Venue selectors:** `market_uid[]`, `polymarket_market_id[]`, and `kalshi_event_ticker[]` are combined with OR logic across parameter types. Example: `polymarket_market_id=123&kalshi_event_ticker=TICKER1` returns markets matching either selector. Results are deduplicated by `market_uid` (if multiple selectors resolve to the same market, it appears once).
...
- **Polymarket slugs/IDs:** Get from Polymarket's API or from events' `venue_ids.polymarket[]`.
- **Kalshi tickers:** Get from Kalshi's API or from events' `venue_ids.kalshi[]`.
...
| `venue_ids.polymarket.market_id` | string | Polymarket market ID (numeric string). Omitted if not on Polymarket. |
